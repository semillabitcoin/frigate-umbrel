...
# Low-latency mempool ingestion via bitcoind ZMQ pubsequence (sub-100ms vs 5s polling).
# Strongly recommended by upstream when backendElectrumServer is set — avoids the
# brief inconsistency window where SP wallets see incorrect amounts during mempool→block.
# Gated on env var so non-Umbrel installs without ZMQ degrade gracefully to polling.
ZMQ_BLOCK=""
